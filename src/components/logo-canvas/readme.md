...
目前工作中主要负责活动页的开发以及维护。在活动页面的制作过程中，发现其与平常的功能页面存在一些不同点：

1. 重 UI 展示，轻逻辑
2. 设计资源多，尤其是图片、视频这类的静态资源
3. 活动页面的 UI 设计基本不可复用

针对第二点最常见的做法就是预加载，先加载第一页所有的静态资源再展示页面。这样做的坏处就是页面 loading 的时间会变长。如果采用一个单调的“菊花”图案展示给用户，会让时间显得更加漫长，所以资源加载期间的 loading 动画如何设计成了我们一个思考的点。

首先，loading 动画要足够的炫，这样，才能吸引用户的眼球。其次，loading 动画不能请求太大的静态资源。最后，loading 动画最好跟公司品牌有一定的连续性，比如，公司的小电视以及“bilibili”。

我们选择了在 logo 上面做文章，打算基于现有的技术框架 vue ，结合 canvas，做一些好玩的事情。由于时间原因，我们没有手动绘制 logo， 而是选择了一张 6kb 的 logo 图片，格式为 png（注意：一定要 png 透明图片，这个在下面会解释），利用 canvas 读取图片数据，然后渲染出动画。由于业务的原因，动画只在 web 平台展示。
...
上面的代码中，有一个 Text 类，这个类是展示 logo 的关键，这个类主要实现了：
...
对于图片数据存储，有多种方法。我们的做法是利用一个 canvas 来存储数据，调用方需要图片数据时，可以直接通过 canvas API 获取。
